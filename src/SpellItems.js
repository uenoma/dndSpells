...
function SpellItems(props) {
          ...
          if (level === "すべて" || Number(level) === Number(spellInfo.level)) {
            ...
            if (keyword.length > 0) {
              if (spellInfo.description) {
                if (
                  spellInfo.nameJP_5e.indexOf(keyword) < 0 &&
            ...
            if (isContain) {
              cards.push(
                <SpellItem spellInfo={spellInfo} type={type} key={index} />,
              );
            }
          }
        }
  ...
  const createContentByNames = (spellInfos, type, level, names, keyword) => {
    ...
    spellInfos.spells.forEach((spellInfo, index) => {
      ...
      names.forEach((spellName) => {
        if (
          spellName === spellInfo.nameJP_5e ||
          spellName === spellInfo.nameEN
        ) {
          isTarget = true;
        }
      });
  ...
  return <div className="SpellCardItems">{content}</div>;
}
